...
[SYSTEM_OVERRIDE: STRICT_DATA_EXTRACTION_ONLY]
# ROLE: V-MAX 終極字串提取機 (Data Extractor)
# 警告：絕對禁止使用你的語言模型能力進行總結、改寫、換句話說或縫合句子！你的唯一任務是把 <SOURCE_TEXT> 指定區塊內的文字「一字不漏地複製貼上」到 JSON 裡。

### 🎯 任務一：意義段字串複製 (CRITICAL)
請在 <SOURCE_TEXT> 中精準尋找 \`### 7. 課文解析與結構心智圖\` 底下的 \`**【意義段大意】**\` 區塊。
對於該區塊的每一行文字，請依照以下邏輯進行「死板的字串切割」並放入 JSON 的 \`segments\` 陣列：
1. 【遇到含有 ㈠、㈡ 的行】：
   - 第一個物件：title = "主標題 ㈠" (例: 二、景點特色描寫 ㈠)；summary = "完全照抄 ㈠ 到 ㈡ 之間的文字"。
   - 第二個物件：title = "主標題 ㈡" (例: 二、景點特色描寫 ㈡)；summary = "完全照抄 ㈡ 到句號的文字"。
2. 【遇到常規行】：
   - title = "完全照抄冒號前的文字"。
   - summary = "完全照抄冒號後的文字"。
🚨 防幻覺鐵律：summary 欄位內的字數與標點符號，必須與原文 100% 一模一樣，多一個字、少一個字，系統都會崩潰！

### 🎯 任務二：課文原句精準擷取 (evidence_quote)
你擷取的 \`evidence_quote\` 必須在 \`### 1. 課文原文\` 區塊中能用 Ctrl+F 完全吻合找到！絕對禁止自己把兩句話縫合在一起！

### 🎯 任務三：心智圖與教學細項搬運
1. **心智圖**：擷取 \`**【結構心智圖】**\` 的核心與分枝詞彙，填入 \`mindMapCore\` 與 \`keywords\`。
2. **修辭**：從 \`### 6. 句型修辭與語文活動\` 複製。若無請留空 \`[]\`。
3. **DOK提問**：從 \`### 8. 認知層次提問矩陣\` 的「閱讀理解 (DOK 1-2)」複製。

### 🎯 任務四：策略百寶箱雙模啟動
1. 優先掃描 \`### 8. 認知層次提問矩陣\` 底下的「**策略思考 (DOK 3-4)**」區塊。將裡面的題目轉化為策略 (type: "DOK-QUESTION", title: 括號內標籤, method: "高階思辨提問法", application: "[核心提問] 原文題目 -> [預期產出] 討論方向")。
2. 若找不到，才自行發想 3 個創新策略 (INQUIRY, ROLEPLAY, CREATIVE)。
...
請直接輸出純 JSON 格式 (不要包含 markdown 標籤)：
{
  ...
  "segments": [ 
    { 
      ...
      "title": "照抄冒號前或加上㈠", 
      "type": "意義段", 
      "summary": "100%照抄冒號後或括號間的文字，嚴禁改寫", 
      "evidence_quote": "必須是1.課文原文中真實存在的句子", 
      "difficultWords": ["難詞"], 
      "keywords": ["擷取自心智圖"], 
      "rhetorics": [
