...
### 💡 【通用主角提取邏輯 (Universal Protagonist Logic)】(CRITICAL)
無論遇到哪一課，請嚴格依據以下 3 點定義來提取「故事主角 (protagonist)」：
1. 具備行動力：主角必須是「推動劇情發展」或「面臨並解決主要衝突」的核心角色。
2. 區分主客體：若課文是「A觀察B得到啟發」，主角是有思考與行動能力的 A，絕對不是被單純觀察的客體 B（例如：人類觀察動植物，主角是人類）。
3. 擬人化判定：除非該動物/物品在故事中會說話、具備人類思想與社交行為（如童話、寓言），否則不可將純動物/死物列為主角。若整篇文章皆無符合條件的主角，請自動切換至 Mode B (無主角)。

### 📤 輸出規範 (Strict JSON)
⚠️ 警告：下方 JSON 僅為「格式範例」，請務必根據【當前實際課文】進行通用法則提取，禁止照抄！

{
  "mode": "Drama Mode" | "Guide Mode",
  "protagonist": {
    "name": "提取出的『核心行動者』名稱 (如無具體主角請填 None)",
    "description": "性格與背景描述 (例如: 善於觀察並解決問題的發明家)",
  ...
  "candidates": [
    {
      ...
      "name": "為本課量身設計的導師名",
      "persona": "語氣晶片 (G1-G6)",
      "description": "他在本課的角色定位",
      "visualDNA": "Age: [明確年齡數字] | [配件與服裝]"
    },
