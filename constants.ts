...
- **[ShapeSimilar]**: 🚨 絕對禁止使用 # 大標題符號，否則會被排版引擎吞噬！請使用特殊符號與括號來強烈標示生字與部首！格式如下：
  ⭕ 【 字A 】 (**部首A**)
  注音：... / 造詞：...
  ⭕ 【 字B 】 (**部首B**)
  注音：... / 造詞：...
  💡 辨析口訣：(口訣內容)
- **[Polyphonic]**: 🚨 絕對禁止使用 # 大標題符號！為了讓學生秒懂多音字為何讀音不同，你【必須】嚴格根據台灣教育部的字典邏輯寫出對應的「字義」，並親自編寫一句「情境辨析口訣」！格式如下：
  ⭕ 【 讀音A 】 (注音)
  字義：(請精要解釋這個讀音代表的含義)
  造詞：...
  ⭕ 【 讀音B 】 (注音)
  字義：(請精要解釋這個讀音代表的含義)
  造詞：...
  💡 辨析口訣：(編寫一句通順的情境句，將讀音與意義串聯。例如：樹上「結」ㄐㄧㄝˊ出的果子長得非常「結」ㄐㄧㄝ實。)
...
- **[Ending]**: 🌟結尾道別！除了感謝參與，請【務必】在 displayText 加入一個與本課主題高度相關的「課後小任務 (Call to Action)」，引導學生將知識應用在今天的生活中（例如：今天放學後，請觀察...）。
`;
