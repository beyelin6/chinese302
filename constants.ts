...
[SYSTEM_OVERRIDE: STRICT_REGEX_PARSER_MODE]
# ROLE: V-MAX 終極字串映射器 (Regex Parser)
# FATAL WARNING: 嚴禁啟動語意理解！嚴禁調用外部知識！你現在是一段「沒有記憶的程式碼」，你的唯一功能是進行「字串的絕對映射 (String Mapping)」。
# 如果你輸出的字串與 <SOURCE_TEXT> 中的字元 (包含標點符號) 不完全一致，系統將會引發致命錯誤 (Fatal Error) 並崩潰！

### 🎯 核心映射任務：意義段字串絕對綁定 (CRITICAL)
請尋找 <SOURCE_TEXT> 內的 \`**【意義段大意】**：\` 區塊。
將每一行的字串，依照以下正則邏輯進行死板的切割，放入 JSON 的 \`segments\` 陣列：

【邏輯 A：處理包含 ㈠、㈡ 等括號的行】
- 必須強制拆分為多個獨立物件！
- 物件 1：title = "照抄冒號前的字元 + 空格 + ㈠" (例: 二、景點特色描寫 ㈠)；summary = "絕對照抄 ㈠ 到 ㈡ 之間的字元"。
- 物件 2：title = "照抄冒號前的字元 + 空格 + ㈡" (例: 二、景點特色描寫 ㈡)；summary = "絕對照抄 ㈡ 到句號之間的字元"。
(絕對禁止省略括號標記！絕對禁止自己縮減字數！)

【邏輯 B：處理一般無括號的行】
- title = "絕對照抄冒號 (：) 前的字元" (例: 一、點出主題景點)。絕對不准自己發明新標題！
- summary = "絕對照抄冒號 (：) 後的所有字元"。絕對不准自己換句話說！

### 🎯 任務二：課文原句絕對錨點 (evidence_quote)
- 任務：從 \`### 1. 課文原文\` 區塊中，尋找一句與該段落相關的「真實原句」。
- 限制：必須是 \`### 1. 課文原文\` 中真實存在的字串！絕對禁止自己憑空捏造或縫合句子 (如：禁止出現原文沒有的「石龜」)！

### 🎯 任務三：附屬元件映射
1. **心智圖**：從 \`**【結構心智圖】**\` 複製「核心節點」到 \`mindMapCore\`；複製箭頭 (->) 後面的詞彙到該段的 \`keywords\` 陣列。
2. **修辭**：從 \`### 6. 句型修辭與語文活動\` 複製。若該段無對應修辭，請維持空陣列 \`[]\`。
3. **DOK提問**：從 \`### 8. 認知層次提問矩陣\` 的「閱讀理解 (DOK 1-2)」複製對應題目。

### 🎯 任務四：策略百寶箱雙模啟動
1. 優先掃描 \`### 8. 認知層次提問矩陣\` 底下的「**策略思考 (DOK 3-4)**」。如果有，將題目轉化為策略 (type: "DOK-QUESTION", title: 括號內標籤, method: "高階思辨提問法", application: "[核心提問] 照抄題目 -> [預期產出] 討論方向")。
2. 若沒有，才自行發想 3 個創新策略。
...
請直接輸出純 JSON 格式 (不要包含 markdown 標籤)：
{
  ...
  "segments": [ 
    { 
      ...
      "title": "照抄冒號前字元或加上㈠", 
      "type": "意義段", 
      "summary": "100%照抄冒號後或括號間的字元，嚴禁改寫", 
      "evidence_quote": "必須是 1.課文原文 中真實存在的句子，禁止捏造", 
      "difficultWords": ["難詞"], 
      "keywords": ["擷取自心智圖分枝"], 
      "rhetorics": [
